...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $activeUsersWithAge = User::join('user_identities', 'users.id', '=', 'user_identities.user_id')
                ...
                ->whereDoesntHave('sentFriendRequests', function ($query) use ($currentUserId) {
                    $query->where('friend_id', $currentUserId)
                        ->where('accepted', false);
                })
                ->whereDoesntHave('receivedFriendRequests', function ($query) use ($currentUserId) {
                    $query->where('user_id', $currentUserId)
                        ->where('accepted', false);
                })
                ->whereDoesntHave('friends', function ($query) use ($currentUserId) {
                    $query->where('friend_id', $currentUserId);
                })
                ->select(
                    'users.id',
